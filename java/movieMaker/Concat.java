...
public class Concat implements ControllerListener, DataSinkListener {
    ...
    String TRANSCODE_MSG = java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("THE_GIVEN_INPUTS_REQUIRE_TRANSCODING_TO_HAVE_A_COMMON_FORMAT_FOR_CONCATENATION.");
	...
	if (inputURL.size() == 0) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("NO_INPUT_URL_IS_SPECIFIED"));
	    prUsage();
	}

	if (outputURL == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("NO_OUTPUT_URL_IS_SPECIFIED"));
	    prUsage();
	}
	...
	for (i = 0; i < inputURL.size(); i++) {
		...
		System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("CANNOT_BUILD_MEDIA_LOCATOR_FROM:_") +" "+ inputURL);
		System.exit(0);
	    }
	}

	if ((oml = createMediaLocator(outputURL)) == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("CANNOT_BUILD_MEDIA_LOCATOR_FROM:_") +" "+ outputURL);
	    System.exit(0);
	}
	...
	if (!concat.doIt(iml, oml)) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("FAILED_TO_CONCATENATE_THE_INPUTS"));
	}
	...
	if ((cd = fileExtToCD(outML.getRemainder())) == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("COULDN'T_FIGURE_OUT_FROM_THE_FILE_EXTENSION_THE_TYPE_OF_OUTPUT_NEEDED!"));
	    return false;
	}
	...
	for (int i = 0; i < inML.length; i++) {
		...
		System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("CANNOT_CREATE_A_PROCESSOR_FROM_THE_GIVEN_URL:_") +" "+ e);
		return false;
	    }
	}
	...
	if (!matchTracks(pInfo, cd)) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("FAILED_TO_MATCH_THE_TRACKS."));
	    return false;
	}
	...
	if (!buildTracks(pInfo)) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("FAILED_TO_BUILD_PROCESSORS_FOR_THE_INPUTS."));
	    return false;
	}
	...
	try {
	    ...
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("FAILED_TO_CREATE_A_PROCESSOR_TO_CONCATENATE_THE_INPUTS."));
	    return false;
	}
	...
	if (!waitForState(p, p.Configured)) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("FAILED_TO_CONFIGURE_THE_PROCESSOR."));
	    return false;
	}
	...
	if ((p.setContentDescriptor(cd)) == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("FAILED_TO_SET_THE_OUTPUT_CONTENT_DESCRIPTOR_ON_THE_PROCESSOR."));
	    return false;
	}
	...
	if (!waitForState(p, p.Realized)) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("FAILED_TO_REALIZE_THE_PROCESSOR."));
	    return false;
	}
...
//	System.err.println("- Start concatenation...");
	...
	try {
	    ...
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("IO_ERROR_DURING_CONCATENATION"));
	    return false;
	}
	...
	for (i = 0; i < pInfo.length; i++) {
		...
		System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("-_FAILED_TO_CONFIGURE_THE_PROCESSOR."));
		return false;
	    }
	...
	if (total[AUDIO] < 1 && total[VIDEO] < 1) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("THERE_IS_NO_AUDIO_OR_VIDEO_TRACKS_TO_CONCATENATE."));
	    return false;
	}
	...
	for (i = 0; i < pInfo.length; i++) {
		...
		for (j = total[type]; j < pInfo[i].numTracksByType[type]; j++) {
		    ...
		    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("-_DISABLE_THE_FOLLOWING_TRACK_SINCE_THE_OTHER_INPUT_MEDIA_DO_NOT_HAVE_A_MATCHING_TYPE."));
	...
	for (type = AUDIO; type < MEDIA_TYPES; type++) {
		...
		if (!tryMatch(pInfo, type, i)) {
		    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("-_CANNOT_TRANSCODE_THE_TRACKS_TO_A_COMMON_FORMAT_FOR_CONCATENATION!__SORRY."));
		    return false;
		}
	    }
	}
	...
	for (int i = 0; i < pInfo.length; i++) {
	    ...
	    // realize the it.
	    if (!waitForState(p, p.Realized)) {
		System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("-_FAILED_TO_REALIZE_THE_PROCESSOR."));
		return false;
	    }
	...
	for (int i = 0; i < supported.length; i++) {
		...
		for (int j = 0; j < pInfo.length; j++) {
			...
			System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("-_TRANSCODING:_") +" "+ pInfo[j].ml);
			System.err.println("  " + oldFmt);
			System.err.println("           "+java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("___________TO:"));
			...
			if (oldSize != null && !oldSize.equals(newSize)) {
			    ...
			    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("-_SCALING:_") +" "+ pInfo[j].ml);
			    System.err.println("  "+java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("__FROM:_") +" "+ oldSize.width +
						" x " + oldSize.height);
			    System.err.println("  "+java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("__TO:_") +" "+ newSize.width +
						" x " + newSize.height);
		...
		if (owner instanceof Codec) {
			...
			if (fmts[j].matches(jpegFmt)) {
			    qc = (QualityControl)cs[i];
	    		    qc.setQuality(val);
			    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("-_SET_QUALITY_TO_") +" "+
					val + " "+java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("_ON_") +" "+ qc);
			    break;
			}
		    }
		}
	...
	if ((ds = p.getDataOutput()) == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("SOMETHING_IS_REALLY_WRONG:_THE_PROCESSOR_DOES_NOT_HAVE_AN_OUTPUT_DATASOURCE"));
	    return null;
	}
	...
	if (evt instanceof ControllerErrorEvent) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("FAILED_TO_CONCATENATE_THE_FILES."));
	    System.exit(-1);
	...
	if (url.startsWith(File.separator)) {
	    if ((ml = new MediaLocator(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("FILE:") + url)) != null)
		return ml;
	} else {
	    String file = java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("FILE:") + System.getProperty("user.dir") + File.separator + url;
    ...
    static void prUsage() {
	System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("USAGE:_JAVA_CONCAT_-O_<OUTPUT>_<INPUT>_..."));
	System.err.println("     "+java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("_____<OUTPUT>:_INPUT_URL_OR_FILE_NAME"));
	System.err.println("     "+java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("_____<INPUT>:_OUTPUT_URL_OR_FILE_NAME"));
	System.exit(0);
    }
	...
	public void setLocator(MediaLocator ml) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Concat").getString("NOT_INTERESTED_IN_A_MEDIA_LOCATOR"));
	}
    }
