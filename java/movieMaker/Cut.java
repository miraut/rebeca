...
public class Cut implements ControllerListener, DataSinkListener {
	...
	boolean frameMode = false; //whether specified in frames or seconds (want seconds) in miliseconds?
	...
	if (inputURL == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("NO_INPUT_URL_SPECIFIED."));
	    prUsage();
	}

	if (outputURL == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("NO_OUTPUT_URL_SPECIFIED."));
	    prUsage();
	}

	if (startV.size() == 0 && endV.size() == 0) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("NO_START_AND_END_POINT_SPECIFIED."));
	    prUsage();
	}
	...
	for (int j = 0; j < start.length; j++) {
	    ...
	    if (prevEnd > start[j]) {
		System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("PREVIOUS_END_POINT_CANNOT_BE_>_THE_NEXT_START_POINT."));
		prUsage();
	    } else if (start[j] >= end[j]) {
		System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("START_POINT_CANNOT_BE_>=_END_POINT."));
		prUsage();
	    }
	...
	if (frameMode) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("START_AND_END_POINTS_ARE_SPECIFIED_IN_FRAMES."));
	} else {
	    ...
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("START:_") +" "+ start[j] +" " +java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("_END:_") +" "+end[j]);
	    }
	}
	...
	if ((iml = createMediaLocator(inputURL)) == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("CANNOT_BUILD_MEDIA_LOCATOR_FROM:_") +" "+ inputURL);
	    System.exit(0);
	}

	if ((oml = createMediaLocator(outputURL)) == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("CANNOT_BUILD_MEDIA_LOCATOR_FROM:_") +" "+ outputURL);
	    System.exit(0);
	}
	...
	if (!cut.doIt(iml, oml, start, end, frameMode)) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("FAILED_TO_CUT_THE_INPUT"));
	}
...
//		System.err.println("Start point cannot be >= end point.");
	...
	///    System.err.println("Start: " + start[j] + " End: " +end[j]);
	    }
	...
	if ((cd = fileExtToCD(outML.getRemainder())) == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("COULDN'T_FIGURE_OUT_FROM_THE_FILE_EXTENSION_THE_TYPE_OF_OUTPUT_NEEDED!"));
	    return false;
	}
	...
	try {
	 //   System.err.println("- Create processor for: " + inML);
	    ...
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("CANNOT_CREATE_A_PROCESSOR_FROM_THE_GIVEN_URL:_")+" " + e);
	    return false;
	}

	//System.err.println("- Configure the processor for: " + inML);
	if (!waitForState(p, p.Configured)) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("FAILED_TO_CONFIGURE_THE_PROCESSOR."));
	    return false;
	}
...
//	System.err.println("- Realize the processor for: " + inML);
	if (!waitForState(p, p.Realized)) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("FAILED_TO_REALIZE_THE_PROCESSOR."));
	    return false;
	}
...
//	System.err.println("- Set output content descriptor to: " + cd);
	if ((p.setContentDescriptor(cd)) == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("FAILED_TO_SET_THE_OUTPUT_CONTENT_DESCRIPTOR_ON_THE_PROCESSOR."));
	    return false;
	}
	...
	try {
	    ...
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("IO_ERROR_DURING_CONCATENATION"));
	    return false;
	}
	...
	for (int i = 0; i < tc.length; i++) {
		...
		if (selected != null) {
		    System.err.println("  "+java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("__TRANSCODE:"));
		    System.err.println("     "+java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("_____FROM:_") +" "+ tc[i].getFormat());
		    System.err.println("     "+java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("_____TO:_") + selected);
		...
		if (owner instanceof Codec) {
			...
			if (fmts[j].matches(jpegFmt)) {
			    qc = (QualityControl)cs[i];
	    		    qc.setQuality(val);
			    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("-_SET_QUALITY_TO_") +" "+
					val +" " +java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("_ON_") +" "+ qc);
			    break;
			}
		    }
		}
	...
	if ((ds = p.getDataOutput()) == null) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("SOMETHING_IS_REALLY_WRONG:_THE_PROCESSOR_DOES_NOT_HAVE_AN_OUTPUT_DATASOURCE"));
	    return null;
	}
	...
	if (evt instanceof ControllerErrorEvent) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("FAILED_TO_CUT_THE_FILE."));
	    //System.exit(-1);
	...
	if (url.startsWith(File.separator)) {
	    if ((ml = new MediaLocator(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("FILE:") + url)) != null)
		return ml;
	} else {
	    String file = java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("FILE:") + System.getProperty("user.dir") + File.separator + url;
...
//	System.err.println("     -f: specify the times in video frames instead of milliseconds");
	...
	public void setLocator(MediaLocator ml) {
	    System.err.println(java.util.ResourceBundle.getBundle("movieMaker/Cut").getString("NOT_INTERESTED_IN_A_MEDIA_LOCATOR"));
	}
    }
