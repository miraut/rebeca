...
public class AnimationPanel extends JComponent
{
  ...
  public void removeAllBefore()
  {
    ...
    for (int i = 0; i <= currIndex; i++)
    {
      ...
      result = f.delete();
      if (result != true)
        System.out.println(java.util.ResourceBundle.getBundle("movieMaker/AnimationPanel").getString("TROUBLE_DELETING_") +" "+
                           nameList.get(i));
      imageList.remove(0);
    }
  }
  ...
  {
    if (imageList.size() == 0)
      g.drawString(java.util.ResourceBundle.getBundle("movieMaker/AnimationPanel").getString("NO_IMAGES_YET!"),20,20);
    else
      draw(g);
  }
