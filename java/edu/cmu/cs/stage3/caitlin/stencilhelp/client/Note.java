...
public class Note implements StencilObject, MouseEventListener, KeyEventListener, StencilObjectPositionListener,
  ...
  public void initializeNote() {
   ...
   protected void generateShapes() {
    ...
    if (hasNext) {
      TextLayout wordLayout = new TextLayout(java.util.ResourceBundle.getBundle("edu/cmu/cs/stage3/caitlin/stencilhelp/client/Note").getString("next"), font, new FontRenderContext(null, false, false));
      AffineTransform textAt = new AffineTransform();
      textAt.translate(bgRect.getX() + bgRect.getWidth() - 43, bgRect.getY() + bgRect.getHeight() + 15);
  ...
  protected Point getClosestPoint(Rectangle rect, Point center) {
    if (rect == null) return new Point(0,0);
        ...
        return new Point( (int) (rect.x + rect.width/2), (int)(rect.y + rect.height + 1));
    } else return new Point( (int)(rect.x + rect.width/2), (int)rect.y + 1);
  }
    ...
    if (scrObject != null) {
      ...
      if (rect != null) {
        if (offset.getY() > 0) {
          ...
          return new Point( (int)(rect.x + rect.width/2), (int)rect.y + 1);
        }
