...
public class Link implements StencilObject, MouseEventListener {
  ...
  public Link(StencilManager stencilManager, ObjectPositionManager posManager, boolean next) {
    ...
    if (next) {
      message = java.util.ResourceBundle.getBundle("edu/cmu/cs/stage3/caitlin/stencilhelp/client/Link").getString("Load_Next_Chapter");
    } else message = java.util.ResourceBundle.getBundle("edu/cmu/cs/stage3/caitlin/stencilhelp/client/Link").getString("Reload_Last_Chapter");
  ...
  protected void createShapes() {
    ...
    if (next) {
      xStart = (int)posManager.getScreenWidth() - width - 20;
      yStart = (int)posManager.getScreenHeight() - 50;
    }
