...
public class StencilManager implements MouseListener, MouseMotionListener, KeyListener, StencilClient, StencilStackChangeListener {
	...
	protected void requestFocus(StencilObject newFocalObject) {
		...
		if ((focalObject != null) && (focalObject instanceof StencilFocusListener) && (stencilFocusListeners.contains((StencilFocusListener) focalObject))) {
			((StencilFocusListener) focalObject).focusLost();
		}
		if ((newFocalObject != null) && (newFocalObject instanceof StencilFocusListener) && (stencilFocusListeners.contains((StencilFocusListener) focalObject))) {
	...
	public void loadStencilsFile() {
		...
		while (loadFile == null) {
			...
			if (fileName != null) {
				...
				if (!(loadFile.exists())) {
					int ans = edu.cmu.cs.stage3.swing.DialogManager.showConfirmDialog(java.util.ResourceBundle.getBundle("edu/cmu/cs/stage3/caitlin/stencilhelp/client/StencilManager").getString("Can't_find_") +" "+ fileName + java.util.ResourceBundle.getBundle("edu/cmu/cs/stage3/caitlin/stencilhelp/client/StencilManager").getString("._Please_choose_another"), java.util.ResourceBundle.getBundle("edu/cmu/cs/stage3/caitlin/stencilhelp/client/StencilManager").getString("Can't_find_file"), javax.swing.JOptionPane.OK_CANCEL_OPTION);
	...
	public void saveStencilsFile() {
		...
		while (saveFile == null) {
			...
			if (fileName != null) {
				...
				//check to see if the file already exists, if not we're fine
				if (saveFile.exists()) {
					String msg = java.util.ResourceBundle.getBundle("edu/cmu/cs/stage3/caitlin/stencilhelp/client/StencilManager").getString("This_file_already_exists,_do_you_want_to_overwrite_it?");
					int ans = edu.cmu.cs.stage3.swing.DialogManager.showConfirmDialog((Object) msg, java.util.ResourceBundle.getBundle("edu/cmu/cs/stage3/caitlin/stencilhelp/client/StencilManager").getString("File_already_exists"), javax.swing.JOptionPane.YES_NO_CANCEL_OPTION);
		...
		if (document != null) {
			Element root = (Element) document.createElement("stencilStack");
	...
	public void loadStencilTutorial(java.io.File tutorialFile) {
		// at this point we should have a valid .stl file to parse, etc.
		//    this.overrideMinResolution = false;
		//
		//    if ( (stencilApp.getScreenSize().width <= 1000) || (stencilApp.getScreenSize().height <= 700 )) {
		//
		//      Object[] options = { "OK", "CANCEL" };
		//      int stop = edu.cmu.cs.stage3.swing.DialogManager.showOptionDialog( "The tutorial works best at resolution of at least 1024 x 768. \nWe recommend that you increase your screen resolution to 1024 x 768\n before continuing with the tutorial.\n If you want to run the tutorial at your current resolution, \nclick OK to continue.", "Screen Resolution Warning", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, options, options[0]);
		//
		//      // handle cancel
		//      if (stop == 1) {
		//        return;
		//      } else {
		//        overrideMinResolution = true;
		//      }
		//      //edu.cmu.cs.stage3.swing.DialogManager.showMessageDialog( "The tutorial needs a resolution of at least 1024 x 768 to run. \nPlease increase your screen resolution, maximize Alice and try again.\n", "Need 1024 x 768", JOptionPane.ERROR_MESSAGE);
		//    }

		// screen is at a reasonable resolution or user says do it anyway.
		...
		this.addMouseEventListener((MouseEventListener) stencilPanel);
		stencilApp.deFocus();
