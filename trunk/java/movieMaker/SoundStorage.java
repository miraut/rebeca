	...
	public void convertTimes(){
		...
		for(int x =0; x<soundList.size(); x++){
			...
			if(time1==time2){
				...
				double percentage = ((double)((SoundData)soundList.get(x)).worldTime - time1)/(time2-time1);
					((SoundData)soundList.get(x)).worldTime=((double)(index-1) + percentage)/16.0;
			}
		}
	}
	...
	public Vector encodeFiles(double length, String exportDirectory) // get
	{
		...
		for (int y = 0; y < soundList.size(); y++) {
			...
			if (sd.data.getExtension().equals("MP3")){
				...
				Sound.convert(orig_file, sound_slice);
			}
			...
			String filename = java.util.ResourceBundle.getBundle("movieMaker/SoundStorage").getString("FILE:") + sound_slice.replace('/', '\\');
			...
			try {			
				...
				AuthoringTool.showErrorDialog( java.util.ResourceBundle.getBundle("movieMaker/SoundStorage").getString("ERROR_ENCODING_SOUND_FILE._")+" ", e);
				return null;
			}
